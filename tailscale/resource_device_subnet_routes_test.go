...
func TestAccTailscaleDeviceSubnetRoutes_WithNodeID(t *testing.T) {
	...
	checkNodeID := func(client *tailscale.Client, rs *terraform.ResourceState) error {
		...
		device, err := client.Devices().Get(context.Background(), rs.Primary.Attributes["device_id"])
		if err != nil {
			return err
		}

		if rs.Primary.Attributes["device_id"] != device.NodeID {
			return fmt.Errorf("state device_id %q does not match node id %q", rs.Primary.Attributes["device_id"], device.NodeID)
		}
...
func TestAccTailscaleDeviceSubnetRoutes_LegacyIDToNodeID(t *testing.T) {
	...
	checkLegacyID := func(client *tailscale.Client, rs *terraform.ResourceState) error {
		...
		if rs.Primary.Attributes["device_id"] != device.ID {
			return fmt.Errorf("state device_id %q does not match legacy id %q", rs.Primary.Attributes["device_id"], device.ID)
		}
