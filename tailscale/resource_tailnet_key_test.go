...
func TestAccTailscaleTailnetKey(t *testing.T) {
	...
	const testTailnetKeySetToDefaults = `
		resource "tailscale_tailnet_key" "test_key" {
			reusable = false
			ephemeral = false
			preauthorized = false
			tags = []
			expiry = 7776000 # but maybe 0
			description = ""
			recreate_if_invalid = ""
		}`
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testTailnetKeyUnsetOptionals,
			},
			{
				Config: testTailnetKeySetToDefaults,
			},
			{
				...
				ImportStateVerifyIgnore: []string{
					"key",                 // sensitive material not returned by the API
					"recreate_if_invalid", // not returned by the API
				},
			},
		},
	})
